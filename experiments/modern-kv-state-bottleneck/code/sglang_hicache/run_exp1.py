"""Experiment 1 (SGLang path): Context Length Scaling.

Drives a public SGLang server over HTTP.  Same design as the vLLM
``run_exp1.py`` (context x residency x reps, prefix ratio fixed at
50%), with results written to ``results/sglang/exp1/<model>/<ctx>/<mode>/``
keeping raw / summary / validation / metadata separation.

Usage:
    python3 sglang_hicache/run_exp1.py \\
        --model qwen --model-path /path/to/model \\
        --context-length 4096 --residency recompute \\
        --output-dir results/sglang/exp1/qwen
"""
...
from sglang_hicache.residency import prepare_cpu_hit, prepare_gpu_hit, prepare_recompute
...
logger = logging.getLogger("sglang-exp1")


def setup_logging(log_dir: str, model_label: str, ctx: int, mode: str) -> Path:
    log_dir = Path(log_dir)
    log_dir.mkdir(parents=True, exist_ok=True)
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    log_file = log_dir / f"sglang-exp1-{model_label}-{ctx}-{mode}-{ts}.log"
...
def _prepare_residency(mode: str, client, workload, snapshot_fn, seed: int):
    """Run the residency preparation for one measured rep."""
    if mode == "recompute":
...
def run_single(args) -> int:
    ...
    setup_logging(args.log_dir, model_label, args.context_length, args.residency)
    ...
    logger.info("=" * 60)
    logger.info("Experiment 1 (SGLang): Context Length Scaling")
    logger.info("Model:     %s (%s)", args.model_id, args.model_path)
    logger.info("Context:   %d tokens", args.context_length)
    logger.info("Residency: %s", args.residency)
    logger.info("Output:    %s", args.output_dir)
    logger.info("Server:    port=%d", server_config.port)
    logger.info("=" * 60)
...
def _run_with_server(args, server_config, client, snapshot_fn) -> None:
    # Build workload through the server-side tokenizer.
    base_text = None
    ...
    for rep in range(args.n_repeats):
        prep = _prepare_residency(
            args.residency, client, workload, snapshot_fn, args.seed
        )
        segment = workload.get_segment(rep)
        before = snapshot_fn()
        result = client.generate(
            segment.prompt_token_ids,
            max_new_tokens=1,
            request_id=200 + rep,
        )
        after = snapshot_fn()
    ...
    # --- summary ---
    valid = [t for t in ttfts if t == t]  # drop NaN
    summary = {
        "runtime": "sglang",
        "model": args.model_id,
        "context_length": args.context_length,
...
def main():
    parser = argparse.ArgumentParser(
        description="Experiment 1 (SGLang): Context Length Scaling"
    )
