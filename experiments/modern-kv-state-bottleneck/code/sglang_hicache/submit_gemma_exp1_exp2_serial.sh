#!/bin/bash
# Submit Gemma 4 Exp1 + Exp2 as one serialized 2xA100 TP=2 chain.
...
PIPELINE_TAG="${PIPELINE_TAG:-$(date -u +%Y%m%dT%H%M%SZ)-exp1-exp2-gemma}"
...
for ctx in 4096 8192 16384 32768; do
    for mode in recompute gpu_hit cpu_hit; do
        label="exp1-gemma-${ctx}-${mode}"
        tag="${PIPELINE_TAG}-${label}"
        submit_one "$label" "$EXP1_RUNNER" \
            "MODEL=gemma,CTX=$ctx,MODE=$mode,TP_SIZE=2,MEM_FRACTION=0.75,RUN_TAG=$tag,SMOKE=0,DRY_RUN=0,HICACHE_RATIO=3,HICACHE_IO_BACKEND=direct,HICACHE_MEM_LAYOUT=page_first_direct"
    done
done
...
for spec in \
    ...
    label="exp2-gemma-32768-${pct}pct-${mode}"
    tag="${PIPELINE_TAG}-${label}"
    submit_one "$label" "$EXP2_RUNNER" \
        "MODEL=gemma,CTX=32768,RATIO=$ratio,MODE=$mode,TP_SIZE=2,MEM_FRACTION=0.75,RUN_TAG=$tag,SMOKE=0,DRY_RUN=0,HICACHE_RATIO=3,HICACHE_IO_BACKEND=direct,HICACHE_MEM_LAYOUT=page_first_direct"
done
