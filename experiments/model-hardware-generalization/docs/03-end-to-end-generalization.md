...
1. Full Configuration 相对 Baseline 的端到端收益是否在四种组合上保持同一方向；
...
Full Configuration 只包含已经在前置实验中通过 capability gate 与 validity gate 的 Hierarchical Cache、I/O Optimization 和 Scheduler Optimization。

本实验不预设 Full Configuration 必须在所有组合上取得正收益。稳定正收益、收益消失、throughput-latency trade-off 与 regression 都作为有效结果保留。
...
这些中间配置不进入所有 workload 的完整主矩阵，只用于解释 Baseline 与 Full Configuration 之间无法由已有机制证据直接解释的差异。
...
该场景用于验证 Full Configuration 在非主要目标 workload 上是否引入固定开销、无效 CPU-tier activity、额外 scheduling overhead 或其他资源竞争。
...
跨硬件主结果优先使用 same-workload comparison，因为本实验最终关心相同实际 workload 在不同 deployment platform 上的系统收益。
...
每类 representative workload 至少覆盖三个 operating regions：

- low load；
- medium load；
- high load / near-saturation。

Low-load point 用于观察 Full Configuration 是否引入 fixed overhead。

Medium-load point 用于观察 cache、I/O 与 scheduler mechanisms 开始发挥作用后的实际收益。

High-load point 用于观察 serving capacity、tail latency 与 saturation behavior。

不同 model × hardware 组合的 absolute saturation request rate 可以不同。因此结果同时保留：

1. same offered workload 下的 operational comparison；
2. 相对各组合自身 saturation capacity 的 normalized operating-region comparison。

实验不把四种组合使用相同 requests/s 作为唯一的可比性标准。
...
- Full Configuration 在某一组合上的收益明显低于其他组合；
- Full Configuration 出现稳定 regression；
...
- system-configuration semantics；
- warm-up protocol；
- measurement window；
- repetition rule；
- processing rule。
...
- driver；
- CUDA/runtime；
- actual GPU reusable-state budget；
- actual CPU-tier budget；
...
每个 `model × hardware × workload × load` point 都同时保存 absolute measurement 和相对自身 Baseline 的 normalized effect。

Throughput gain 定义为：

```text
throughput_gain = (throughput_full - throughput_baseline) / throughput_baseline
```

Latency reduction 使用：

```text
latency_reduction = (latency_baseline - latency_full) / latency_baseline
```

跨组合 robustness 主要比较 normalized gain 的方向与幅度，而不是直接比较 Qwen、Gemma、A100 与 L40 的 absolute throughput。

所有 normalized result 必须能够回溯到其对应 absolute baseline、full measurement 与 uncertainty summary。

## 11. 实验执行流程

首先从前置实验冻结三类 representative workload、load grid 与 point identifiers。

随后确认四种 model × hardware 组合均通过对应 runtime/state capability gate。

每种组合先执行 Baseline calibration，确认 workload materialization、target operating region 与 instrumentation 有效。

正式 paired runs 在相同 logical trace 上执行 Baseline 与 Full Configuration。

每个正式 point 在统一 warm-up 后执行多次独立重复测量，并保存完整 raw measurement 与 metadata。

不同配置的执行顺序尽量交替或随机化，降低节点长期状态变化造成的系统偏差。

已有 A100 results 只有在 model/runtime revision、trace、system semantics、cache budget、measurement rule 与当前冻结配置完全一致时才直接复用。否则补跑 matched A100 point。

主矩阵执行完成后，再依据预定义触发条件决定是否执行 targeted attribution runs。
...
- Baseline 与 Full Configuration semantics 明确；
- Full Configuration 所包含的 mechanisms 已通过 capability gate；
...
- offered-load schedule 未因 runtime behavior 被静默修改；
...
Capability 缺失不能解释为 optimization failure。

## 13. 结果判断逻辑

### 情况 A：四种组合均获得稳定收益

如果 Full Configuration 在主要 long-context 与 mixed workload 中均取得稳定正收益，同时 short-context 没有 material regression，则说明经过验证的 Strata 类 mechanisms 具有较强的 model × hardware robustness。

该结果只支持 systems mechanism 跨当前两个模型和两个平台的 robustness，不扩张为对任意模型或任意 GPU 的普遍结论。

### 情况 B：收益方向一致，但幅度不同

如果四种组合的 normalized effect 方向一致，但 gain magnitude 明显不同，则结论为 mechanism generalizes but benefit magnitude is model/hardware sensitive。

差异由 Experiments 1–2 的 state pressure、I/O、scheduler 与 hardware-pressure evidence 解释。

### 情况 C：某些组合收益消失，同时对应 bottleneck 减弱

如果某个组合上 Full Configuration 几乎无额外收益，而前置实验同时显示其 cache/state、I/O 或 scheduling bottleneck 已明显减弱，则该结果属于 applicability boundary。

该结果解释为优化必要性下降，而不是 mechanism 被反证。

### 情况 D：Bottleneck 仍存在，但 Full Configuration 无收益

如果前置实验明确显示目标 bottleneck 仍然存在，而 Full Configuration 没有形成 end-to-end gain，则触发 targeted attribution。
...
如果 Full Configuration 提高 throughput，但 P99 TTFT 或 request completion time 稳定恶化，则结果标记为 `throughput_latency_tradeoff`，不能写成无条件 system improvement。
...
| Model | GPU | Workload | Load region | Baseline | Full Config | Throughput gain | TTFT change | Mechanism evidence | Conclusion |
|---|---|---|---|---|---|---:|---:|---|---|
...
跨硬件时，mechanism 是否仍然成立
        ↓
Experiment 3
这些 mechanisms 最终能否形成稳定的 serving gain
```
