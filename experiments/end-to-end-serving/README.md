...
Mixed workload 额外要求按 long-context 与 short-context request class 分别报告 latency 与 throughput，避免 aggregate metrics 掩盖 cross-class interference。
...
Experiment 3 采用 representative primary matrix 加 targeted robustness checks，而不是对 workload composition、cache locality、output-length heterogeneity 和 offered load 做完整笛卡尔积。这样保留主要结论，同时避免不必要的实验爆炸。
